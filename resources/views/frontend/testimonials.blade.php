<!doctype html>
<html lang="en">
  <head>
    <title>Wholesome Craft | Contact Us</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="keywords" content="">
    <meta name="author" content="">

   

   <!-- include navigation -->
   @include("frontend.includes.nav-links")
   @include("frontend.includes.navigation")
  <!-- include navigation -->





  <section class="home_search spacetop">
   <div class="container-fluid container-who">
     <div class="row">
       <div class="col-12 mt-4">
         <img src="img/testicover.png" class="img-fluid topcoverimage" alt="">
       </div>

       <div class="col-12 tctext">
         <h1 class="mb-3">What Our Customers <br>
Are Saying About Us</h1>
        <p>Lorem ipsum dolor sit amet, consectetur <br> adipiscing elit maecenas convallis.</p>

        <a href="#" class="btn btn-round shadow">
              <img src="img/arrowdown2.svg" alt="">
          </a>
       </div>
     </div>
   </div>
</section>



<!--  -->
<section class="glogreen3">
  <div class="container-fluid container-who">
    <div class="row pt-5 centerit">
      <div class="col-12 col-md-12 col-lg-7">
         <img src="img/textinames.svg" class="img-fluid" alt="">
      </div>

      <div class="col-12 col-md-12 col-lg-5 pr-md-5">
        <div>
      <div class="owl-carousel  owl-testimo owl-theme">

        @foreach ($testimonials as $testimonial)
            
          <div class="item">
            <div class="card-testi">
              <h3 class="whotext2">“{!! $testimonial->content !!}”</h3>
              <h5 class="whotext2 m-3">- {{ ucwords($testimonial->fullname) }} </h5>
            </div>
          </div>

        @endforeach
        
        {{-- <div class="item">
            <div class="card-testi">
            <h3 class="whotext2">“I absolutely loveee wholesome craft! They’re the best at what they do, and I’ve never felt better.”</h3>
            <h5 class="whotext2 m-3">- Amelie James, 29</h5>
          </div>
        </div>

        <div class="item">
            <div class="card-testi">
            <h3 class="whotext2">“I absolutely loveee wholesome craft! They’re the best at what they do, and I’ve never felt better.”</h3>
            <h5 class="whotext2 m-3">- Amelie James, 29</h5>
          </div>
        </div> --}}

       
    </div>
      </div>
        
      </div>

     
    </div>
  </div>
</section>
<!--  -->





















<!-- contact down part -->
 <section class="glomag3">
   <div class="container-fluid  ">
     <div class="row  centerit">
        <div class="col-12 col-md-6  leftsps ">
        <h2 class="my-5">Our Awesome Services Are <br> Just Click Awaye</h2>
        <p>Our  services comprise of meal plans that make it <br> easy for you to live your best life and live healthy. <br> We can cook the meals and deliver to your <br> doorstep.</p>

        <ul class="list-inline">
        <li class="list-inline-item">
          <a href="#"><img src="img/applestore.svg"  width="130"  alt=""></a>
        </li>
        <li class="list-inline-item">
          <a href="#"><img src="img/googleplay.svg"  width="130"    alt=""></a>
        </li>
      </ul>
        </div>

        <div class="col-12 col-md-6 px-0">
        <div class="jkk">
          <img src="img/conatctdrop.png" class="img-fluid" alt="">
        </div>
        </div>
        
     </div>
   </div>
 </section>

<!-- contact down part -->





<!-- Our awesome services are just click away -->
<!-- <section class="golpad ">
     <div class="container-fluid container-who2  py-5">
       <div class="row centerit">
          

          <div class="col-12 col-md-5 pl-md-5">
          <img src="img/petal.svg" class="img-fluid" width="40" alt="">
                <h2 class="my-3">Our Awesome Services Are Just Click Awaye</h2>

      <p  class="py-4">Our  services comprise of meal plans that make it easy for you to live your best life and live healthy. We can cook the meals and deliver to your doorstep.</p>
  
      <ul class="list-inline">
        <li class="list-inline-item">
          <a href="#"><img src="img/applestore.svg"  width="130"  alt=""></a>
        </li>
        <li class="list-inline-item">
          <a href="#"><img src="img/googleplay.svg"  width="130"    alt=""></a>
        </li>
      </ul>

            </div>

            <div class="col-12 col-md-7">
             <img src="img/roundgreen.svg" class="w-100" alt="">
          </div>


            <div class="col-12 ">
              <img src="img/phonescreen1.png" class="img-fluid" alt="">
            </div>

          
       </div>
     </div>
  </section> -->
<!-- Our awesome services are just click away -->









<!-- footer includes -->
<div class="pushupfot">
  @include("frontend.includes.footer")
</div>

@include("frontend.includes.footer-links")
 <!-- footer includes -->

  </body>
</html>